      ...
      <div className={styles.globalAddCatalogContainer}>
        <div className={styles.main}>
          <div style={{ textAlign: 'center', padding: '60px' }}>
            ...
            <h2 style={{ color: '#0040D6', marginBottom: '8px' }}>Catalog Added Successfully!</h2>
    ...
    <div className={styles.globalAddCatalogContainer}>
      <div className={styles.main}>
        ...
        <div className={styles.top}>
          ...
          <p>Add the product photos and we will auto-fill some of the product description</p>

          {/* ── STEPS ── */}
          <div className={styles.row}>
  <div className={styles.steps}>
    {/* STEP 1 — Select Category (active until product type chosen) */}
    <div className={`${styles.step} ${!hasAttributes ? styles.active : ''}`}>
      {hasAttributes ? <span className={styles.check}>✔</span> : <span>1&nbsp;</span>}
      Select Category
    </div>
    {/* STEP 2 — Add Product Details (active after product type chosen) */}
    <div className={`${styles.step} ${hasAttributes ? styles.active : ''}`}>
      <span>2&nbsp;</span>
      Add Product Details
    </div>
...
/>
        ...
        {error && (
          <div style={{
            background: '#fff0f0', border: '1px solid #E51300',
            borderRadius: '8px', padding: '12px 16px',
            marginBottom: '16px', color: '#E51300', fontSize: '14px'
          }}>
            ⚠ {error}
          </div>
        )}
        ...
        {noAttributes && (
          <div style={{
            ...
            borderRadius: '8px', padding: '12px 16px',
            marginBottom: '16px', color: '#7a5c00', fontSize: '14px'
          }}>
            ⚠ This product type has no attributes configured yet. Please select a different product.
          </div>
        )}
            ...
            <div className={styles.left}>
              ...
              <div className={styles["info-box"]}>
                ...
                <p className={styles["info-text"]}>
                  Mandatory fields are marked with * and must be filled before submitting.
                </p>
              </div>
              ...
              <div className={styles.listing}>
                {fixedFields.map(({ key, label, required }) => (
                  <div className={styles.line} key={key}>
                    <span className={`${styles.pill} ${required ? styles.required : ''}`}>
                      {label}{required ? ' *' : ''}
                    </span>
                    <input
              ...
              <div className={styles.listing}>
                {Object.entries(fieldAttributes).map(([key, rule]) => {
                  ...
                  const options = isDropdown ? rule.filter(v => v !== '*') : [];

                  return (
                    <div className={styles.line} key={key}>
                      <span className={`${styles.pill} ${isRequired ? styles.required : ''}`}>
              ...
              <div className={styles.card1}>
                ...
                <div className={styles["image-grid"]}>
                  {Object.entries(imageAttributes).map(([key, rule]) => {
                    ...
                    return (
                      <div key={key} className={styles["img-box"]} style={{ height: 'auto', padding: '12px' }}>
                ...
                <button className={styles["blue-btn"]} onClick={() => document.getElementById('imageUpload').click()}>
                    + Add more image
                </button>
              </div>
            </div>
